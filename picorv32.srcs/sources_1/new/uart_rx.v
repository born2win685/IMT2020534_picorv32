...
module uart_rx #(
    ...
    always @(posedge i_clk) begin
        if(i_en) begin
            if(i_reset)
                for(rst_idx = 0; rst_idx < OVERSAMPLE; rst_idx=rst_idx+1)
     ...
     function automatic obtained_bit(input [OVERSAMPLE-1:0] oversampled_bits);
        ...
        begin
            count = 0;
            for(idx = MAJORITY_START_IDX; idx <= MAJORITY_END_IDX; idx=idx+1)
    ...
    always @(posedge i_clk) begin
      ...
      always @(posedge i_clk) begin
        if(i_en) begin
            if(i_reset)
                for(frame_rst_idx = 0; frame_rst_idx <= N_DATA_BITS; frame_rst_idx=frame_rst_idx+1)
